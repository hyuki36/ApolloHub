...
// Neu script gan sonSlug (Son Studio): loader co cho dien key cua user.
function buildRawLoader(host, entry) {
  const s = String(entry.slug || entry.id).replace(/[^a-z0-9_-]/gi, '');
  if (entry.sonSlug) {
    return 'local _KEY = "PASTE-YOUR-KEY-HERE"\n'
      + 'loadstring(game:HttpGet("https://' + host + '/raw/' + s + '.lua?key=" .. _KEY))()';
  }
  return 'loadstring(game:HttpGet("https://' + host + '/raw/' + s + '.lua"))()';
}
...
module.exports = { clientKind, isBrowser, isRobloxUa, getUa, getHost, buildRawLoader, buildPayloadLoader, sendBlank, sendLua, safeEqual };
